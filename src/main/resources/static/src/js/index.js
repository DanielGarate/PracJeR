	...
	create() {
		...
		this.startButton.on('pointerdown', () => {
			...
			loadUsers(function(users) {
				...
				//When users are loaded from server
				for (var i = 0; i < users.length; i++) {
					if (users[i].name == player2Name && users[i].password != ' ') {
						console.log("Usuario repetido");
						existe = true;
					} else if (users[i].name == player2Name && users[i].password == ' ') {
						console.log("Bienvenido " + player2Name);
						existe = true;
					}
				}
				if (!existe) {
...
class User1SignIn extends Phaser.Scene {
	...
	preload() {
		...
		//this.load.html('nameform', '../../resources/htmls/loginform.html');
    	this.load.image('pic', '../../resources/img/PANTALLAFINAL.png');
	}
...
class Password1SignIn extends Phaser.Scene {
	constructor() {
	...
	create() {
		...
		this.add.text(80, 365, player1Name, { font: '16px Courier', fill: '#ffff00' });
		var textEntry1 = this.add.text(80, 400, '', { font: '16px Courier', fill: '#ffff00' });
...
class User2SignIn extends Phaser.Scene {
	constructor() {
		super({ key: 'user2signin' });
		this.logIn2Button = new button(this, 'password2signin', 230, 540, 'buttonGo');
	...
	preload() {
		...
		this.load.image("pantallaSignIn", "../../resources/img/PANTALLAINIC.png");
		this.logIn2Button.preload();
	}


	create() {
		this.add.image(400, 300, "pantallaSignIn");  //Añade un nuevo elemento del juego 
		this.add.text(80, 350, 'P2 Enter your name', { font: '16px Courier', fill: '#ffffff' });

		this.logIn2Button.create();



		//https://phaser.io/examples/v3/view/input/keyboard/text-entry
		var textEntry2 = this.add.text(80, 365, '', { font: '16px Courier', fill: '#ffff00' });
		...
		this.input.keyboard.on('keydown', function(event) {
			...
			if (event.keyCode === 8 && textEntry2.text.length > 0) {
				if (textEntry2.text.length > 0) {
					...
					player2Name = textEntry1.text;
				}
			}
			else if (event.keyCode >= 48 && event.keyCode < 90) {
				textEntry2.text += event.key;
			}
		});
...
class Password2SignIn extends Phaser.Scene {
	constructor() {
		super({ key: 'password2signin' });
		this.logIn2Button = new LogIn2Button(this, 'game', 230, 540, 'buttonGo');
	...
	create() {
		this.add.image(400, 300, "pantallaSignIn");  //Añade un nuevo elemento del juego 
		this.add.text(80, 350, 'P2 Enter your name', { font: '16px Courier', fill: '#ffffff' });
		this.add.text(80, 385, 'P2 Enter your password', { font: '16px Courier', fill: '#ffffff' });
		...
		//https://phaser.io/examples/v3/view/input/keyboard/text-entry
		this.add.text(80, 365, player2Name, { font: '16px Courier', fill: '#ffff00' });
		var textEntry2 = this.add.text(80, 400, '', { font: '16px Courier', fill: '#ffff00' });
		...
		this.input.keyboard.on('keydown', function(event) {
			...
			if (event.keyCode === 8 && textEntry2.text.length > 0) {
				if (textEntry1.text.length > 0) {
					...
					textEntry2.text = textEntry2.text.substr(0, textEntry2.text.length - 1);
					player2Password = textEntry1.text;
				} else {
					textEntry2.text = textEntry2.text.substr(0, textEntry2.text.length);
...
class Game extends Phaser.Scene {
	...
	create() {
		...
		name2 = this.add.text(player2.x, player2.y, player2Name);
...
class menuIni extends Phaser.Scene {
	constructor() {
		...
		this.button = new button(this, 'user1signin', 230, 410, 'buttonPl');
